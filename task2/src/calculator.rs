...
impl Calculator {
    ...
    pub fn subtraction(&mut self, x: i64, y: i64) -> Option<i64> {
        let r = OperationType::perform(&OperationType::Subtraction, x, y);
        if r.is_some() {
            self.history
    ...
    pub fn multiplication(&mut self, x: i64, y: i64) -> Option<i64> {
        let r = OperationType::perform(&OperationType::Multiplication, x, y);
        if r.is_some() {
            self.history
    ...
    pub fn show_history(&self) -> String {
        ...
        for (i, op) in self.history.iter().enumerate() {
            let line = format!(
                ...
                op.second_num,
                op.operation_type
                    .perform(op.first_num, op.second_num)
                    .unwrap()
                    .to_string()
            );
            r += &line;
